...
The responses are applied to the whole `(t, polarisation, node)` array. Responses which have an array-level ("batched") form are converted to it automatically ([`Nonlinear.batched_response`](@ref)): the Kerr responses (pointwise, or as broadcasts over the components for vector fields), the plasma response ([`Nonlinear.PlasmaCumtrapzBatched`](@ref)) and both Raman responses ([`Nonlinear.RamanPolarFieldBatched`](@ref) and [`Nonlinear.RamanPolarEnvBatched`](@ref)); these are threaded over the nodes on the CPU and run as whole-array kernels on a device. Any other response is applied node by node on the host (serially) and cannot run on a device.

The design of the fixed rule, the measurements behind its defaults, the evidence for its accuracy and the device (GPU) execution path are described in detail in the developer guide, [The fixed-quadrature modal transform and the device (GPU) path](@ref).

...
As a reference point, on the RDW VUV example (`examples/simple_interface/RDWemission_VUV_gradient.jl`: four ``\mathrm{HE}_{1m}`` modes, 1.5 m of a He pressure gradient, ≈4500 steps) with 8 threads on a laptop, the adaptive cubature took 18.6 min and the fixed rule 1.6/2.3/3.8/6.4 min at ``n_r`` = 64/128/256/512. With the smooth (`sum_integral`) PPT rate the fixed rule at 32 and 128 nodes and the adaptive cubature all agree to ``10^{-8}`` in the final field (56 s at 32 nodes); with the default PPT rate the final fields differ at the 1–2 % level (relative L2 of ``|E|^2``) between all rules, the adaptive one included, which is the quadrature noise of the kinked model rather than a property of either transform.
